// =============================
// FRONTEND LOGIC (Full v3 — Fixed User Totals)
// =============================
...
if (connectBtn) {
  connectBtn.addEventListener("click", async () => {
    if (!window.ethereum) return alert("Please install MetaMask, Token Pocket, Trust Wallet!");
...
if (stakeBtn) {
  stakeBtn.addEventListener("click", async () => {
    ...
    if (isNaN(amount) || amount <= 0) return alert("Enter a valid amount (≥ 1 POL).");

    // Referral address
    let ref = new URLSearchParams(window.location.search).get("ref");
    if (!ref || !web3.utils.isAddress(ref)) {
      ref = "0xA9873d0de2Fe82d4eEA749aFcd084cFC83874C3E"; // default ref
    }

    const valueWei = web3.utils.toWei(amount.toString(), "ether");
    const gasLimit = 550000;  // safe and sufficient for most stake/withdraw functions
    const gasPrice = await web3.eth.getGasPrice();  // fetches the current network gas price
...
if (withdrawBtn) {
  withdrawBtn.addEventListener("click", async () => {
    ...
    const gasLimit = 550005;  // safe and sufficient for most stake/withdraw functions
    const gasPrice = await web3.eth.getGasPrice();  // fetches the current network gas price
...
async function updateUI() {
  ...
  try {
    ...
    setText(balanceEl, `${parseFloat(web3.utils.fromWei(bal, "ether")).toFixed(4)} POL`);

    // Contract balance
    const contractBal = await contract.methods.msgvalue().call();
    setText(contractBalanceEl, `${parseFloat(web3.utils.fromWei(contractBal, "ether")).toFixed(4)} POL`);

    // Withdrawable
    const withdrawable = await contract.methods.getUserAvailable(userAccount).call();
    setText(withdrawableEl, `${parseFloat(web3.utils.fromWei(withdrawable, "ether")).toFixed(4)} POL`);

    // Global Total Staked
    const totalStaked = await contract.methods.totalStaked().call();
    setText(totalStakedEl, `${parseFloat(web3.utils.fromWei(totalStaked, "ether")).toFixed(4)} POL`);

    // ✅ User Total Deposits (correct getter)
    const totalDeposits = await contract.methods.getUserTotalDeposits(userAccount).call();
    setText(totalDepositsEl, `${parseFloat(web3.utils.fromWei(totalDeposits, "ether")).toFixed(4)} POL`);

    // ✅ User Total Withdrawn (correct getter)
    const totalWithdrawn = await contract.methods.getUserTotalWithdrawn(userAccount).call();
    setText(totalWithdrawnEl, `${parseFloat(web3.utils.fromWei(totalWithdrawn, "ether")).toFixed(4)} POL`);
